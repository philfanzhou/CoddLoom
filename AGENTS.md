...
2. 检查 PR 对应的远端 issue。已经完成的 issue 应关闭；只完成一部分或仍有后续工作的 issue
   应保持开启，并更新说明或链接后续 issue，不能因为 PR 已合并就放任状态失真。
3. 检查远端工作分支。没有明确保留理由的已合并分支应删除；需要保留时必须说明原因。
4. 检查本地分支和远端跟踪引用。在确认改动已经安全进入目标分支后，切回并更新目标分支，删除
   不再使用的本地工作分支，并清理已经失效的远端跟踪引用。
5. 向用户汇报 PR、issue、远端分支和本地分支各自的最终状态；任何未完成的清理都要说明原因和
   后续动作。
